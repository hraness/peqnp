...
The [indexed-transfer experiment](experiments/indexed-transfer.md) replaces pair enumeration with a sorted index that derives the same units in the same order. On 160 fresh cases it cuts preprocessing from 11,235,212 to 1,048,871 events and finishes four cases the generic matcher could not, yet still costs more than no preprocessing on every case: 1,452,179 events against a 520,161 baseline. The [implication-calibration experiment](experiments/implication-calibration.md) builds the standard 2-SAT implication graph instead. On 120 two-variable-clause cases it recovers all 173 forced literals where the two-clause library finds 63, including every long-chain case the library cannot start; its linear-time decision with an explicit certificate still costs more declared events than the toy solver on 102 of 120 cases. The [fragment-interface experiment](experiments/fragment-interface.md) moves to 162 mixed 2/3-CNFs where search is expensive and reads only the binary fragment through that graph. The interface delta stays negative on 140 cases and turns positive on 22, all but one of them cases where the fragment's own contradiction decided the whole formula without search. The [extraction-cost experiment](experiments/extraction-cost.md) then makes the read cheaper: one stamped search state with inheritance and settling cuts extraction by a third on 176 fresh cases and on the 162 matched ones, flips five matched cases and no fresh one, and does so under a worst-case bound that no known algorithm improves to linear. These are finite research results, not a polynomial SAT solver. `bun run check` checks Rust and ledger behavior and reproduces all six artifacts byte for byte.

Formulas with at most 12 variables are labelled by a truth table. Above that bound the reference is a declared oracle: [CaDiCaL](https://github.com/arminbiere/cadical) 3.0.1, pinned by path and SHA-256 in `src/oracle.rs`, run once under a conflict limit named by the protocol, with every SAT model checked against the raw clauses and every UNSAT answer checked as an LRAT proof by the repository's own std-only checker; a conflict-limit unknown stays unknown. Its answers are committed in `artifacts/<experiment>-oracle.json`, and the artifacts are produced by a replay that re-checks every model and proof without invoking any solver, so the solver is optional: without it `bun run check` verifies every artifact by replay and prints that oracle regeneration was skipped. To regenerate answer files, install it with `brew install cadical`, confirm the pin with `cargo run --locked --release -- oracle`, and run `cargo run --locked --release -- oracle-answers <experiment>`; the registered `demo` experiment over three fixtures exercises the path, and `cargo run --locked --release -- demo out.json --oracle-answers tests/oracle/demo-oracle.json` replays it. A different version, or a different build of the same version (a digest mismatch with a matching `--version`, which a different Homebrew bottle or a different build produces), means the pin, not the machine, must be reviewed.
