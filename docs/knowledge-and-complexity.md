...
After the existing calibration, exhaustively solve a tiny training corpus. Extract candidate backbones or short entailed clauses; infer reusable rule schemas; prove each schema sound independently. Freeze the library and apply it to unseen instances and adversarial families. Count extraction, training, lookup, rule application, intermediate growth, and residual search separately. Compare against the same deterministic solver without the learned library. Preserve failures and distinguish finite observations from general proofs.
