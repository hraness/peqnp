...
Use the pinned Rust toolchain and Bun 1.3.14. Install dependencies with `bun install --frozen-lockfile --ignore-scripts`, run focused tests, then `bun run check`. Obtain independent review for changes to the language, evaluator, reference oracle, or scoring. The final continuous integration check must pass for the delivered commit. A change to an oracle-answer file (`artifacts/<experiment>-oracle.json`) must additionally pass `PEQNP_REQUIRE_ORACLE=1 bun run check` on the host that produced it, where the pinned solver regenerates the file byte for byte; CI has no solver and verifies such files by replay only.
