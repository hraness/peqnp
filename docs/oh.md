...
`oh:record:research:v6` adds the reviewed argument in `scripts/research-records-v6.mjs`: counting the loading of algorithms and background knowledge changes constants, never classes; length-dependent knowledge is advice; only amortization survives as a measurable quantity. It requires the v2 and v5 records to exist already.
