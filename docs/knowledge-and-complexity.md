...
Ben Guo suggested that this bears on P versus NP. This project adopts the operational part of the proposal and gives it a precise computational reading. It takes no position on the article's metaphysics of non-physical or agential patterns, which the experiments here cannot test.
...
all in one declared cost model. Report any one-time acquisition of R's rules separately and amortize it only over an explicit batch. Δ is Levin's effort-versus-competency delta restricted to computation. The clue-transfer pilot measured Δ < 0 on every one of 122 cases: the library's clues were sound and reduced search, but reading them through pair enumeration cost about four times the baseline. By the article's own refutation criterion that interface fails. Two further experiments, each under a protocol frozen before its corpus existed, tested one interface for cheaper reading and one for wider coverage. The [sorted index](../experiments/indexed-transfer.md) reads the same clues for about one tenth of the generic matcher's cost and still has Δ < 0 on all 160 fresh cases. The [implication graph](../experiments/implication-calibration.md) reads every forced literal of a 2-CNF, including the F_k chains the library cannot start, and its linear-time decision with an explicit certificate still has Δ < 0 against the toy solver on 102 of 120 cases. The common cause is that the residual search on these small formulas was already cheap, so no T_build and T_read could be repaid. A positive Δ needs formulas whose residual search is expensive and an interface whose build cost is already paid.

**What the framework cannot supply.** Levin argues that no formal model fully captures a physical system, so a built machine may exhibit patterns its designers did not account for. A P versus NP claim needs the opposite discipline: the cost model is closed, and every operation that builds or reads a clue is charged inside it. A contribution absent from the accounting cannot appear in a proof. Where the article looks for unaccounted competencies, this project looks for accounted ones and records the difference. The connection is a research heuristic and a measurement rule, not a proof step.
...
5. The target is unchanged. A representation with Δ > 0 on every 3-SAT input, polynomial build and read costs, and a polynomial residual would be the algorithm the constructive target asks for. None is known.
