...
export const DOCUMENT_PATHS_V1 = [
...
export function documentRecords(documents, previousRegistry = null) {
  if (documents.length !== DOCUMENT_PATHS_V1.length || documents.some((doc, index) => doc.path !== DOCUMENT_PATHS_V1[index] ||
...
export function documentRecordsV2(documents, previousRegistry = null) {
  ...
  return [
    ...
    record(DOCUMENT_REGISTRY_PREFIX + canonicalSha256({ documents: entries, previousRegistry, schema }), "context", {
      schema,
      authority: "Canonical project research narratives; working Markdown must match these editions.",
      documents: entries,
      previousRegistry,
